        ...
        <div class="row">
            <div class="col-md-8 mt-2 mb-4">
                ...
                @if (!empty($post->posts_video))
                    <iframe width="100%" height="400" src="https://www.youtube.com/embed/{{ $post->posts_video }}"
                ...
                @foreach ($ads as $ad)
                    @if ($ad->type == 1 && $ad->category_id == 23)
                        <a href="{{ $ad->link }}"> <img class="img-fluid pb-1 pt-2 lazyload float-left mr-2 "
                                width="728" height="90" src="{{ asset($ad->ads) }}"></a>
                ...
                <div class="wrapper">
                    ...
                    <div class="navbar-social">
                        ...
                        <a href="https://www.facebook.com/sharer.php?u={{ URL::to('/haber-' . str_slug($post->title_tr) . '-' . $post->id) }}"
                            class="icon-button facebook"><i class="icon-facebook fa fa-facebook"></i><span></span></a>
                ...
                @foreach ($comments as $comment)
                    <div class="col-md-12 bg-light  border-bottom">
                        ...
                        <p class="">
                            ...
                            <i class="fa fa-envelope"></i> {{ $comment->details }}<br />
                        </p>
                ...
                <form action="{{ route('add.comments', $post->id) }}" method="post">
                    ...
                    <div class="form-group w-100">
                        ...
                        <input type="Text" class="form-control" aria-describedby="emailHelp" placeholder="İsim Soyisim"
                            name="name">
                        <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone
                            else.</small>
                    </div>
                    <div class="form-group w-100 h-100">
                        <textarea class="form-control" name="details" placeholder="Yorumunuzu Giriniz" rows="3"></textarea>
                    </div>
                    <div class="row mt-2">
        ...
        <script type="application/ld+json">
{
   ...
   "@type":"NewsArticle",
   "@id":"{{ url('/')."/"."haber"."-".Str::slug($post->title_tr)."-"."$post->id" }}",
   "url":"{{ url('/')."/"."haber"."-".Str::slug($post->title_tr)."-"."$post->id/" }}#newsarticle",
   "headline":"{{ $post->title_tr }}",
   "mainEntityOfPage":"{{ url('/')."/"."haber"."-".Str::slug($post->title_tr)."-"."$post->id" }}",
   ...
   "articleBody":"{!! json_encode($cleanedContent) !!}",
   "image":{
      "@type":"ImageObject",
      "url":"{{ url('/').'/'.$post->image }}",
      "width":600,
